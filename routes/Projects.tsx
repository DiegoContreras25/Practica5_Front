...
import { Cookie, Film } from "../types.ts";
...
const ProjectsPage = (props: PageProps<ProjectsProps>) => {
  ...
  if (!data) {
    return <h1>No se han encontrado proyectos o pelis!</h1>; // Si no hay proyectos o films, mostramos un mensaje
  }

  return (
    <div>
      <a href="/" class="HomeButton">home</a>
      <h1 class="ProjectsMainTitle">Proyectos</h1>
      {data.projects.map((project) => (
        <div key={project.project}>
          <h2 class="ProjectsName">{project.project}</h2>
          <Film_All
            films={data.films.filter((film) =>
              project.film_IDs.includes(film._id)
            )}
          />
        </div>
      ))}

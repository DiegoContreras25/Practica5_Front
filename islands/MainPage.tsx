...
import { NameFilter, SelectFilter } from "./Filters.tsx";
...
const MainPage: FunctionComponent<Params> = ({ films_original }) => {
  ...
  useEffect(() => {
    const uniqueBrands = Array.from(
      new Set(films_original.map((film) => film.brand)),
    ).sort();
    const uniqueIso = Array.from(
      new Set(films_original.map((film) => film.iso)),
    ).sort((a, b) => a - b);
  ...
  const filter_format = (films: Film[]) => {
    ...
    if (formato_signal.value == "ThirtyFive") {
      ...
      return films.filter((film) =>
        film.formatThirtyFive == true && film.formatOneTwenty == true
      );
    } else {
      return films;
    }
  ...
  const filter_nombre = (films: Film[]) => {
    ...
    return films.filter((film) =>
      film.name.includes(name_signal.value.toLowerCase())
    );
  };
  ...
  useEffect(() => {
    setFilms(
      filter_marca(
        filter_iso(filter_format(filter_color(filter_nombre(films_original)))),
      ),
    );
  }, [
    color_signal.value,
    formato_signal.value,
    iso_signal.value,
    brand_signal.value,
    name_signal.value,
  ]);
  ...
  return (
    <>
      <h1 class="Title">Pelis de Diego</h1>
      <div class="Filters">
        <a href="/projects" class="ProjectsButton">Proyectos</a>
        <SelectFilter
          options={brands}
          selected={brand_signal}
          FilterType={"marca"}
        />
        <SelectFilter options={iso} selected={iso_signal} FilterType={"iso"} />
        <SelectFilter
          options={format_options}
          selected={formato_signal}
          FilterType={"formato"}
        />
        <SelectFilter
          options={color_options}
          selected={color_signal}
          FilterType={"color"}
        />
